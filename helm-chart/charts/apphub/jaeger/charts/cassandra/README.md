...
```bash
helm install --namespace "cassandra" -n "cassandra" <helm-repo>/cassandra
```
...
```bash
helm install --namespace "cassandra" -n "cassandra" --set config.cluster_size=5 <helm-repo>/cassandra/
```
